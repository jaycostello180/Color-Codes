// map-view.js - A minimally invasive implementation that preserves original functionality
(function() {
  // Run when DOM is ready
  document.addEventListener('DOMContentLoaded', function() {
    console.log('Minimally invasive map implementation loaded');
  ...
  // Add map button without disrupting other functionality
  function addMapButton() {
    const viewControls = document.querySelector('.view-controls');
    if (!viewControls) return;
    
    // Only remove buttons with duplicate map views, not spectrum or other views
    const mapButtons = viewControls.querySelectorAll(
    ...
    // Only remove buttons that specifically have "Map" in their text
    // but keep the original "Map View" button for Galaxy View
    mapButtons.forEach(btn => {
      if (btn.textContent.includes('Map') && 
          btn.textContent !== 'Map View' && 
          btn.getAttribute('data-view') !== 'map') {
        btn.remove();
      }
    });
    
    // Create our map button only if it doesn't already exist
    if (!document.querySelector('[data-view="location-map"]')) {
      ...
      mapButton.textContent = 'Location Map';
      
      // Add click handler that doesn't interfere with the original renderView
      mapButton.addEventListener('click', function() {
        ...
        // Directly render the map without overriding renderView
        renderLocationMap();
      });
      
      viewControls.appendChild(mapButton);
      console.log('Location Map button added');
    }
  }
  ...
  function renderLocationMap() {
    ...
    loadLeaflet()
      ...
      .catch(error => {
        ...
        document.getElementById('retry-map-btn').addEventListener('click', renderLocationMap);
      });
  }
